...
import com.suqi.center.machine.dao.mapper.YunMemberDeviceMapper;
...
@Service
@Primary
public class YunMemberDeviceServiceImpl extends ServiceImpl<YunMemberDeviceMapper, YunMemberDevice> implements IYunMemberDeviceService {

    @Override
    public Page<YunMemberDevice> quickPageById(boolean queryTotals, Page<YunMemberDevice> page, long lastId, QueryWrapper<YunMemberDevice> wrapper) {
    ...
    @Override
    public Page<YunMemberDevice> quickPage(Page<YunMemberDevice> page, QueryWrapper<YunMemberDevice> wrapper) {
        long total = 0L;
        boolean overrideSelect = false;
        String sqlSelect = wrapper.getSqlSelect();
        if (sqlSelect != null && !"".equals(sqlSelect) && sqlSelect.indexOf(",") > 0) {
